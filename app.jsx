...
function nodeListLines(n) {
  if (!n || !n.leafK) return 0;
  let c = 0;
  for (const k of n.leafK) c += 1 + ((k.rolled && k.rolled.length) || 0);
  return c;
}
...
function buildOrder(bom, excluded, tops) {
  ...
  function walk(pn) {
    ...
    seen[pn] = 1;
    if (!isConsumablePN(pn)) for (const k of (bom.children[pn] || [])) walk(k.pn);
...
function segHitsAny(x1, y1, x2, y2, obs) {
  ...
  for (const o of obs) {
    if (sx2 < o.x || sx1 > o.x + o.w || sy2 < o.y || sy1 > o.y + o.h) continue;
    return true;
  }
  return false;
}
function findClearX(pref, y1, y2, obs, dir) {
  for (let s = 0; s <= 60; s++) {
    ...
    if (!segHitsAny(x, y1, x, y2, obs)) return x;
  }
  return pref;
}
function findClearY(pref, x1, x2, obs, dir) {
  for (let s = 0; s <= 40; s++) {
    ...
    if (!segHitsAny(x1, y, x2, y, obs)) return y;
  }
  return pref;
}
...
function autoSides(p, c, pBottom) {
  ...
  if (c.y >= pBottom - 4) return { out: hasList ? (ccx <= pcx ? "left" : "right") : "bottom", in: "top" };
  if (c.y + c.h <= p.y + 4) return { out: "top", in: "bottom" };
...
function SheetDrawing({ bom, sheet, purchased, fit, qaField }) {
  ...
  if (L.bullet) {
    ...
    for (const n of L.flat) {
      ...
      bels.push(<text key={"p" + key} x={bx0 + NW / 2} y={pnBase} textAnchor="middle" fontFamily={FONT} fontSize={fPN} fontWeight={800} fill={isTop ? C.navy : C.navy2}>{n.pn}</text>);
      descLines.forEach((l, li) => bels.push(<text key={"d" + key + li} x={bx0 + NW / 2} y={descTop + dLH * (li + 0.8)} textAnchor="middle" fontFamily={FONT} fontSize={fD} fontWeight={600} fill="#333">{l}</text>));
      ...
      if (n.hasBullets) {
        ...
        bels.push(<line key={"rail" + key} x1={railX} y1={by - 1} x2={railX} y2={by + totalLines * BLH - 3} stroke="#bbb" strokeWidth={.8} />);
        let li = 0;
        n.leafK.forEach((k, i) => {
          const ly = by + li * BLH + 8; li++;
          ...
          (k.rolled || []).forEach((r, ri) => {
            const ry = by + li * BLH + 8; li++;
            const rlabel = "› " + r.pn + "  " + truncate(r.desc, compChars) + "  (" + (r.qty || "1") + ")";
            bels.push(<text key={"br" + key + i + "_" + ri} x={railX + 20} y={ry} fontFamily={FONT} fontSize={fBullet * 0.94} fontWeight={400} fill="#555">{rlabel}</text>);
          });
        });
      }
      ...
      if (n.asmK && n.asmK.length) {
        ...
        n.asmK.forEach(k => {
          ...
          const obs = [];
          for (const o of OBS) {
            if (o.pn !== n.pn && o.pn !== k.pn) obs.push(o.box);   // other boxes
            if (o.list && o.pn !== k.pn) obs.push(o.list);         // lists except the target's
          }
          ...
          bels.push(<path key={"cn" + key + k.pn} d={d} fill="none" stroke={C.navy} strokeWidth={1.2} strokeLinejoin="round" />);
          ...
          bels.push(<path key={"ar" + key + k.pn} d={`M${last[0]} ${last[1]} L${last[0] - ux * s - uy * s * 0.6} ${last[1] - uy * s + ux * s * 0.6} L${last[0] - ux * s + uy * s * 0.6} ${last[1] - uy * s - ux * s * 0.6} Z`} fill={C.navy} />);
        });
      }
      if (outside) {
        const slice = bels.splice(nStart);
        bels.push(<g key={"off" + key} className="dw-offframe" opacity={0.3}>{slice}</g>);
      }
    }
    ...
    for (const n of L.flat) {
      ...
      minX = Math.min(minX, bx - 14); minY = Math.min(minY, by - 14);
    ...
    const svg = <svg viewBox={`${minX} ${minY} ${vbW} ${vbH}`} preserveAspectRatio="xMidYMid meet"
      style={fit ? { width: Math.min(vbW, cap), maxWidth: "100%", height: "auto", display: "block", margin: "0 auto" }
                 : { width: vbW * z, height: "auto", display: "block", margin: "0 auto" }}
      xmlns="http://www.w3.org/2000/svg">{border}{bels}</svg>;
...
function DraggableSvg({ svg, W, H, editable, nudges, setNudges, fit, cap, anchors, setAnchors, fillBox, setView }) {
  ...
  useEffect(() => {
    ...
    const onCtx = e => {
      const rect = e.target.closest && e.target.closest(".dw-draggable"); if (!rect) return;
      const pn = rect.getAttribute("data-pn"); if (!pn) return;
      ...
      const cleanup = () => { if (menu.parentNode) document.body.removeChild(menu); document.removeEventListener("mousedown", away); };
      const away = ev => { if (!menu.contains(ev.target)) cleanup(); };
...
function TreeDoc({ bom, excluded, tops, cfgName, m, purchased, profile, customer, sheetSize, editable, nudges, setNudges, anchors, setAnchors, textScale, zoom, showBorder, fit, setFit, view, setView }) {
  ...
  const D = (() => {
    ...
    return { ...b,
      ...
      NH: Math.round(Math.max(b.NH * tA,
        Math.max(3, b.fontPN * tA * 0.35) + b.fontPN * tA
  ...
  return (
    <Sheet wide={oneSheet} fixed={oneSheet ? SHEET_TABLOID : null}>
      {/* ---- drawing title block ---- */}
      <div style={{ border: `1.5px solid #111`, marginBottom: oneSheet ? 6 : 12, flex: "0 0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 250px" }}>
          <div style={{ padding: oneSheet ? "4px 8px" : "8px 10px", borderRight: "1px solid #111" }}>
            {P.id === "island"
              ? <img src={ISLAND_LOGO} alt="Island Components" style={{ height: oneSheet ? 24 : 34, display: "block" }} />
          ...
          <div style={{ padding: oneSheet ? "4px 8px" : "8px 10px", textAlign: "center", borderRight: "1px solid #111" }}>
            <div style={{ fontWeight: 800, fontSize: oneSheet ? 13 : 16 }}>{(bom.parts[tops[0]] && bom.parts[tops[0]].desc ? bom.parts[tops[0]].desc : tops[0]).toUpperCase()}</div>
            <div style={{ fontSize: oneSheet ? 11 : 13, letterSpacing: ".1em", marginTop: 1 }}>FAMILY TREE</div>
            <div style={{ fontSize: oneSheet ? 9 : 10, marginTop: 2 }}>TOP LEVEL ASSEMBL{tops.length > 1 ? "IES" : "Y"}: <b style={{ fontFamily: MONO }}>{tops.join(" + ")}</b> — {cfgName}</div>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "150px 1fr 250px", gap: 6, marginBottom: 4, fontSize: oneSheet ? 6.8 : 8, lineHeight: oneSheet ? 1.25 : 1.45, flex: "0 0 auto" }}>
    ...
    <Sheet>
      <div style={{ border: `1.5px solid #111`, marginBottom: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 250px" }}>
          ...
          <div style={{ padding: oneSheet ? "4px 8px" : "8px 10px", textAlign: "center", borderRight: "1px solid #111" }}>
            <div style={{ fontWeight: 800, fontSize: oneSheet ? 13 : 16 }}>{(topPart.desc || tops[0]).toUpperCase()}</div>
            <div style={{ fontSize: 13, letterSpacing: ".1em", marginTop: 2 }}>PARTS LIST</div>
            <div style={{ fontSize: oneSheet ? 9 : 10, marginTop: 2 }}>TOP LEVEL ASSEMBL{tops.length > 1 ? "IES" : "Y"}: <b style={{ fontFamily: MONO }}>{tops.join(" + ")}</b> — {cfgName} · SEE FAMILY TREE F-{tops.join("+")}</div>
          </div>
          <div style={{ fontSize: 8.5 }}>
        ...
        <div style={{ flex: 1, minWidth: 320, display: "flex", flexDirection: "column", maxHeight: "calc(100vh - 46px)" }}>
          ...
          {generated && (
            <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 16px", borderBottom: `1px solid ${C.line}`, background: "#FCFCFA", flexWrap: "wrap" }}>
              ...
              {tab === "tree" && sheetSize === "tabloid" && (
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8, marginLeft: 4, paddingLeft: 8, borderLeft: `1px solid ${C.line}` }}>
