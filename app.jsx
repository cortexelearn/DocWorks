      ...
      <div style={{ border: `1.5px solid #111`, marginBottom: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 250px" }}>
          <div style={{ padding: "8px 10px", borderRight: "1px solid #111" }}>
            ...
            <div style={{ fontSize: 8.5, marginTop: 2 }}>EZ MOTORS, INC.</div>
            <div style={{ fontSize: 7.5, color: "#555" }}>AEROSPACE ELECTROMECHANICAL SYSTEMS</div>
            <div style={{ fontSize: 7.5, color: "#555" }}>CAGE CODE: 0Z9K8</div>
          </div>
          ...
          <div style={{ fontSize: 8.5 }}>
            {[["DOCUMENT NO.", "F-" + tops.join("+"), "REV."], ["DATE", m.date, m.rev || "1"], ["DRAWN BY:", "Engineering", nSheets + " SHEET" + (nSheets > 1 ? "S" : "")], ["CHECKED BY:", "Quality", ""], ["APPROVED BY:", "", ""]].map((r, i) => (
    ...
    <Sheet>
      <div style={{ border: `1.5px solid #111`, marginBottom: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 250px" }}>
          ...
          <div style={{ fontSize: 8.5 }}>
            {[["DOCUMENT NO.", "P-" + tops.join("+"), "REV."], ["DATE", m.date, m.rev || "1"], ["DRAWN BY:", "Engineering", "1 OF 1"], ["CHECKED BY:", "Quality", ""], ["APPROVED BY:", "", ""]].map((r, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "82px 1fr 60px" }}>
                <div style={{ ...tbC, fontWeight: 700 }}>{r[0]}</div>
                <div style={{ ...tbC, fontFamily: MONO }}>{r[1]}</div>
                <div style={{ ...tbC, borderRight: "none", textAlign: "center", fontWeight: i === 0 ? 700 : 400 }}>{r[2]}</div>
    ...
    return (
      <Sheet key={pn}>
        ...
        <ol style={{ fontSize: 10.5, margin: "0 0 12px 18px" }}>
          {["Sample traveler. Drawing requirements, torque values, cure schedules, test limits, and process specifications require Engineering and Quality approval before production use.",
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 14 }}>
          <thead><tr>
            <th style={th}>Op</th><th style={th}>Dept</th>
            <th style={{ ...th, width: "32%" }}>Operation / Detailed Work Instruction</th>
            <th style={{ ...th, width: "22%" }}>Acceptance / Data to Record</th>
            <th style={{ ...th, width: 44 }}>Qty Pass</th><th style={{ ...th, width: 44 }}>Qty Rej</th>
            <th style={th}>Operator / Date</th><th style={th}>QA / Date</th>
          </tr></thead>
          <tbody>
            {ops.map(o => (
              <tr key={o.op}>
                ...
                <td style={{ ...td, width: 66, fontSize: 9, textTransform: "uppercase" }}>{o.dept}</td>
                <td style={td}>{o.text}</td>
                <td style={td}>{o.accept}</td>
                <td style={{ ...td, width: 44, verticalAlign: "bottom" }}><div style={{ borderBottom: "1px solid #bbb", height: 13 }} /></td>
                <td style={{ ...td, width: 44, verticalAlign: "bottom" }}><div style={{ borderBottom: "1px solid #bbb", height: 13 }} /></td>
...
function DocWorks() {
  ...
  const importProject = f => { const rd = new FileReader(); rd.onload = () => { try {
      ...
      const mm = j.meta || {}; setWo(mm.wo || ""); setSn(mm.sn || ""); setProg(mm.prog || "Sample Program"); setDate(mm.date || date); setRev(mm.rev || "1"); setEco(mm.eco || "ECO-0001"); setChange(mm.change || "INITIAL RELEASE (GENERATED)");
  ...
  return (
    <div style={{ fontFamily: '"Segoe UI",system-ui,sans-serif', background: C.backdrop, color: C.ink, fontSize: 14, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ background: C.navy, color: "#fff", padding: "10px 18px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", fontSize: 10.5, opacity: .65, fontFamily: MONO }}>v0.9.1 PROTOTYPE</div>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 320, display: "flex", flexDirection: "column", maxHeight: "calc(100vh - 46px)" }}>
          ...
          <div style={{ overflowY: "auto", padding: 22, display: "flex", flexDirection: "column", alignItems: "center", gap: 22, flex: 1 }}>
            ...
            {generated && tab === "tree" && <TreeDoc bom={bom} excluded={generated.excluded} tops={generated.tops} cfgName={generated.cfgName} m={m} purchased={generated.purchased || {}} />}
            {generated && tab === "plist" && <PartsListDoc bom={bom} excluded={generated.excluded} tops={generated.tops} cfgName={generated.cfgName} m={m} purchased={generated.purchased || {}} />}
            {generated && tab === "trav" && <TravelerDocs bom={bom} excluded={generated.excluded} tops={generated.tops} m={m} />}
            {generated && tab === "wi" && <WIDocs bom={bom} excluded={generated.excluded} tops={generated.tops} m={m} />}
          </div>
        </div>
      </div>
    </div>
